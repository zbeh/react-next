...
colord(backgroundColor).isDark()?'#ddddd':'#33333'
...
const ColorBox: React.FC<{ name: string; color: string }> = ({name,color,}) => {
  return <div className="w-96 h-60 flex-col justify-center items-center text-center uppercase" style={{backgroundColor:color,color:getTextColor(color)}}>
   <span>{name}</span>
   <span>{color}</span>
  </div>;
};
